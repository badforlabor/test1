		...
		if(null != currentAccount && FrontiaAccount.Type.USER == currentAccount.getType()) {
		    ...
		    if(!MediaType.BAIDU.toString().equals(user.getPlatform())) {
		        authorization.bindBaiduOAuth(this, list, new AuthorizationListener() {
                    ...
                    public void onFailure(int errCode, String errMsg) {
                        ...
                        view.setText("获取百度token失败，错误为:"+errCode+""+errMsg+"，只有获取百度token才能使用个人文件存储功能，请返回重新尝试登录");
                        setContentView(view);
                    }
                    ...
                    public void onCancel() {
                        TextView view = new TextView(PersonalFileActivity.this);
                        view.setText("只有获取百度token才能使用个人文件存储功能，请返回重新尝试登录");
                        setContentView(view);
                    }
		    ...
		    authorization.authorize(this,MediaType.BAIDU.toString(),list,new AuthorizationListener() {
		        ...
		        public void onFailure(int arg0, String arg1) {
		            TextView view = new TextView(PersonalFileActivity.this);
		            view.setText("登录百度账号失败，错误为:"+arg0+""+arg1+"，只有获取百度token才能使用个人文件存储功能，请返回重新尝试登录");
		            setContentView(view);
		        }
